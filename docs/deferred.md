...
### H1 — `trade-monitor` 주기적 import 스파이크 — ⚠️ **재정정: 스파이커 맞음 (원 H1 방향 옳았음)**

> **2026-06-23 3차 — "무죄"는 과교정이었음.** 새 대시보드에서 trade-monitor가 **CPU 1위(29.56%)** 로 잡힘.
> `kubectl top` 순간값은 0m(자는 중) ↔ 대시보드 29% = **전형적 톱니파**. 원 H1의 "60초 톱니파" 직관이 맞았고,
> 1·2차에서 "무죄"로 본 건 측정창이 sleep 구간에 걸린 timing artifact. 단 **주범은 매분 콜드스타트가 아니라**
> warm Deployment가 **매 사이클 `python -m` 재포크 → matplotlib/pandas import를 ~235s마다 반복**한 것.
> → S1으로 **in-process 루프(import 1회)** 적용해 완화. 아래는 원 가설 기록(보존).
...
### S1 — trade-monitor: in-process 루프로 import 1회화  *(H1 대응 — ✅ 2026-06-23 적용)*

- 1차 조치(036772f): CronJob → warm Deployment(`sh -c 'while…; python -m trade_monitor; sleep 180'`).
  **하지만 매 사이클 python을 재포크 → import 비용이 그대로 남아 스파이크 지속**(H1 재정정 참조).
- **2차 조치(이번):** args를 **python in-process 루프**로 교체 — `python -u -c`로 모듈을 **부팅 시 1회 import**,
  이후 `main()`만 반복 호출. cadence는 `sleep 120`(2분). [deployment.yaml](../workloads/trade-monitor/deployment.yaml)
  - 안전장치: chart_renderer가 `plt.close(fig)` → figure leak 없음. `SIGALRM(55)`로 기존 `timeout 55`
    hung-run kill 보존. 예외는 catch → 한 사이클 실패가 루프를 안 죽임.
  - 기대 효과: **import 폭발 제거 → 스파이크 진폭이 fetch+렌더 수준으로 급감** (import가 비용의 대부분이었음).
  - 이미지 수정 불필요(`trade_monitor.main.main` 재사용). 검증: kustomize 빌드 OK + 파드에서 ast.parse/import 통과.

> (원래 S1 차선책 기록) 매분 반복 import 콜드스타트 제거가 핵심 효과.
...
- **실측 효과 (재시작 후):** children 합 **198m → 53m (−73%)**, parent 49m→26m(스트림 수신↓ 부수효과),
  메모리 ~190Mi→~96Mi. netdata 전체 **247m → 79m.** 팬 연속 성분 거의 소멸.
- ⚠️ **rollout 함정 (해소함):** `netdata-config`가 `disableNameSuffixHash:true`였어서 ArgoCD가
  ConfigMap 내용만 갱신하고 파드 재시작을 안 함(subPath는 live-reload 안 됨 + netdata는 부팅 시 1회만
  conf 읽음) → "Synced인데 미발효". 최초 적용은 수동 `rollout restart`로 발효시켰고, 재발 방지로
  **해시를 다시 켜서**(kustomization에서 `disableNameSuffixHash` 제거) 앞으론 conf 변경 시 자동 rollout.
...
- 2026-06-23(적용 완료): child.conf `update every=5` 발효(수동 rollout), **children 198m→53m 확인**.
  `disableNameSuffixHash` 제거로 향후 conf 변경 자동 rollout화 — netdata 설정 변경의 고질적 미발효 함정 해소.
- 2026-06-23(3차): 새 대시보드에서 **trade-monitor CPU 1위(29.56%)** + longhorn 28.34% 동시 포착,
  netdata는 8.45%로 하락 확인(S4 효과). **H1 재정정**(무죄→주기 스파이커, 원 직관 옳음). 원인은 매 사이클
  python 재포크로 인한 import 반복. **S1 2차 조치 적용**: in-process 루프(import 1회) + cadence 2분.
- **남은 액션:** (1) S1 sync 후 trade-monitor 스파이크 진폭 하락 확인(대시보드 history). (2) H5 longhorn —
  스파이크 순간 `kubectl top -n longhorn-system`으로 작업 확정 → 필요시 S5(fast-check 체크섬 주기↓).
  단 longhorn은 데이터 보호 작업이라 trade-monitor 잡고도 팬 시끄러울 때만 손댐.
